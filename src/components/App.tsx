...
import InfoTooltip, { TMessageData } from "./InfoTooltip";
...
function App() {
  ...
  const [tooltipStatus, setTooltipStatus] = React.useState<TMessageData | null>(
    null
  );
  ...
  return (
    <div className="page__content">
      ...
      <Routes location={modal || location}>
        ...
        <Route
          path="/card/:id"
          element={<ImagePopup onClose={() => navigate("/")} />}
        />
        <Route
          path="*"
          element={
      ...
      {modal && (
        <Routes>
          <Route
            path="/card/:id"
            element={<ImagePopup onClose={() => navigate(-1)} />}
          />
        </Routes>
      )}
    </div>
